...
/**
 *  Implementation of function fn:year-from-date.
 */
public class YearFromDate extends Function
{    
    static Prototype[] protos = { 
        Prototype.fn("year-from-date", XQType.INTEGER.opt, Exec.class).arg("op", XQType.DATE.opt)
    };
    ...
    public static class Exec extends Function.IntegerCall
    {
            ...
            throws EvaluationException
        {
            ...
            if (v instanceof MomentValue) {
                ...
                return mv.getValue().getYear();
            }
            try {
                Date dt = Date.parseDate(v.getString());
                return dt.getYear();
            }
            catch (DateTimeException e) {
                context.error(Conversion.ERR_CAST, this,
                              "cannot cast to xs:date : " + e.getMessage());
                return 0;
            }
        }
    }
